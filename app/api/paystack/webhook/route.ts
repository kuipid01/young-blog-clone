...
export async function POST(req: NextRequest) {
  ...
  if (!WEBHOOK_SECRET) {
    console.error(
      "PAYSTACK_WEBHOOK_SECRET is not set. Check environment variables."
    );
  ...
  if (hash !== signature) {
    console.warn(
      `Paystack Webhook: Invalid signature. Request rejected. Hash: ${hash}, Signature: ${signature}`
    );
  ...
  console.log(
    `Paystack Webhook Received: ${event.event} for reference: ${event.data.reference}`
  ); // LOG 1
  ...
  try {
    switch (event.event) {
      case "charge.success":
        ...
        if (!userId) {
          console.error(
            `Webhook Error [${reference}]: Charge success event missing required metadata (user_id).`
          ); // LOG 2
        ...
        await db.transaction(async (tx) => {
          ...
          const expectedNewBalance =
            Number(previousBalance) + Number(amountInCurrency);
    ...
    console.error(
      "FATAL Error processing Paystack webhook event:",
