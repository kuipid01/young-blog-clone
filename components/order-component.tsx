...
import { Facebook, Download, Eye, Search, Filter, ChevronDown, X } from "lucide-react"
...
const orders: Order[] = [
  {
    id: "ORD-2025120301",
    product: "Czech Republic FACEBOOK (30-5000 friend) (market very good) (No 2fa)",
    format: "uid|pass|email|pass email|cookie",
    price: 9353.0,
    quantity: 2,
    total: 18706.0,
    status: "completed",
    date: "2025-12-03 14:32:00",
  },
  {
    id: "ORD-2025120202",
    product: "EGYPT FACEBOOK (30+ friends) YEAR 2017-2023 (ACCOUNT LOCATION IN NIGERIA)",
    format: "uid|pass|2fa|email|pass email",
    price: 5845.0,
    quantity: 1,
    total: 5845.0,
    status: "completed",
  ...
  {
    id: "ORD-2025120103",
    product: "USA FACEBOOK (50-500 friends) (Marketplace Enabled) (No 2fa)",
    format: "uid|pass|email|pass email|cookie",
    price: 12500.0,
    quantity: 3,
    total: 37500.0,
  ...
  {
    id: "ORD-2025113004",
    product: "UK FACEBOOK (100+ friends) YEAR 2015-2020 (Strong Accounts)",
    format: "uid|pass|2fa|email|cookie",
    price: 15000.0,
    quantity: 1,
    total: 15000.0,
    status: "completed",
    date: "2025-11-30 11:20:00",
  },
  {
    id: "ORD-2025112905",
    product: "Germany FACEBOOK (30-200 friends) (No 2fa) (Marketplace)",
    format: "uid|pass|email|pass email",
    price: 8200.0,
    quantity: 5,
    total: 41000.0,
    status: "failed",
...
Format: ${order.format}
Price per unit: ₦${order.price.toLocaleString("en-NG", { minimumFractionDigits: 2 })}
Quantity: ${order.quantity}
Total Price: ₦${order.total.toLocaleString("en-NG", { minimumFractionDigits: 2 })}
Date: ${order.date}

--- Mock Data Content ---
Account 1: <DATA_1_FOLLOWING_FORMAT>
Account 2: <DATA_2_FOLLOWING_FORMAT>
... (up to ${order.quantity} accounts)
  ...
  link.download = `${order.id}_${order.product.substring(0, 30).replace(/[^a-z0-9]/gi, "_")}.txt`
...
export function OrdersContent() {
  ...
  const getStatusBadge = (status: Order["status"]) => {
    switch (status) {
      case "completed":
        return <span className="px-3 py-1 text-xs font-medium rounded-full bg-green-100 text-green-700">Completed</span>
      case "pending":
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-500 mt-1">View and manage your purchase history</p>
      </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                <th className="text-left px-6 py-4 text-sm font-semibold text-gray-600 whitespace-nowrap">ORDER ID</th>
                <th className="text-left px-6 py-4 text-sm font-semibold text-gray-600">PRODUCT</th>
                <th className="text-left px-6 py-4 text-sm font-semibold text-gray-600 whitespace-nowrap">QTY</th>
                <th className="text-left px-6 py-4 text-sm font-semibold text-gray-600 whitespace-nowrap">TOTAL</th>
                ...
                filteredOrders.map((order) => (
                  <tr key={order.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="text-sm font-medium text-violet-600">{order.id}</span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-start gap-3 max-w-md">
                        ...
                        <div>
                          <p className="text-sm font-medium text-gray-900 line-clamp-2">{order.product}</p>
                          <p className="text-xs text-gray-500 mt-0.5">{order.format}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 flex justify-center items-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg">
            <div className="flex justify-between items-center p-6 border-b border-gray-200">
              ...
              <button onClick={closeViewDetails} className="p-1 rounded-full hover:bg-gray-100">
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-base font-semibold text-gray-900">{selectedOrder.product}</p>
                </div>
                <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-base text-gray-700">{selectedOrder.date}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-500">Quantity</p>
                  <p className="text-base text-gray-700">{selectedOrder.quantity}</p>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-sm font-medium text-gray-500">Price/Unit</p>
                  <p className="text-base text-gray-700">
                    ₦{selectedOrder.price.toLocaleString("en-NG", { minimumFractionDigits: 2 })}
                  </p>
                </div>
                <div>
              ...
              <div>
                  <p className="text-sm font-medium text-gray-500">Delivery Format</p>
                  <code className="text-sm text-gray-700 bg-gray-100 p-2 rounded-md inline-block mt-1">{selectedOrder.format}</code>
              </div>
            </div>
