...
export function ReferralContent() {
  ...
  const referralCode = "KUIPID2025"
  const referralLink = `https://jemilmarketplace.com/register?ref=${referralCode}`
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-violet-600 to-purple-600 rounded-2xl p-6 text-white">
        ...
        <p className="text-violet-100 max-w-xl">
          Earn <span className="font-bold text-yellow-300">2%</span> of every deposit made by users you refer. Share
          your unique link and start earning passive income today!
        </p>
      </div>
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
        ...
        <div className="space-y-4">
          {/* Referral Link */}
          <div>
            <label className="text-sm text-gray-500 mb-2 block">Share this link with friends</label>
          ...
          {/* Referral Code */}
          <div>
            <label className="text-sm text-gray-500 mb-2 block">Your Referral Code</label>
            <div className="flex gap-2">
              <input
                type="text"
                value={referralCode}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-5 border-b border-gray-100">
          <h2 className="text-lg font-semibold text-gray-800">Referral Earnings History</h2>
        </div>
              ...
              {referralHistory.length > 0 ? (
                referralHistory.map((item, index) => (
                  <tr key={index} className="hover:bg-gray-50">
                    ...
                    <td className="px-5 py-4 text-sm text-gray-600">₦{item.deposited.toLocaleString()}</td>
                    <td className="px-5 py-4 text-sm font-semibold text-green-600">+₦{item.earned.toLocaleString()}</td>
